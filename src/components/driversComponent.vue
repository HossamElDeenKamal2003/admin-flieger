...
export default {
    ...
    methods: {
        async increase(id) {
        try {
            const response = await axios.patch(`https://backend.fego-rides.com/admin/increase/${id}`);
        ...
        async getLocations() {
            try {
                const response = await axios.get("https://backend.fego-rides.com/admin/get-location");

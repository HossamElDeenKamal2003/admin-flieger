...
export default {
    ...
    methods: {
        async fetchPrices() {
            try {
                const response = await axios.get('https://backend.fego-rides.com/prices/level4/getprices');
        ...
        async handleSubmit() {
            if (this.editMode) {
                try {
                    const response = await axios.patch('https://backend.fego-rides.com/prices/level4/updateprices', this.form);
                ...
                try {
                    const response = await axios.post('https://backend.fego-rides.com/prices/level4/putprices', this.form);
                    alert(response.data.message);
                    this.fetchPrices();
                } catch (error) {
        ...
        deletePrice(country) {
            ...
            if (confirm(`Are you sure you want to delete prices for ${country}?`)) {
                ...
                axios.post('https://backend.fego-rides.com/prices/delete/level4', {
                    country: country
                }).then(() => {
                    this.fetchPrices();
                }).catch(error => {
